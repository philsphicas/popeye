...
typedef struct
{
    ...
    move_effect_journal_index_type previous_movement;
                         ...
                         char const *closing_sequence)
{
  ...
  context->previous_movement = move_effect_journal_index_null;
...
static void write_departure(output_plaintext_move_context_type *context,
                            move_effect_journal_index_type movement)
{
  if (context->previous_movement!=move_effect_journal_index_null
...
static void write_pre_capture_effects(output_plaintext_move_context_type *move_context)
{
  move_effect_journal_index_type curr;
  move_effect_journal_index_type const base = move_effect_journal_base[nbply];

  for (curr = base; curr<base+move_effect_journal_index_offset_capture; ++curr)
    switch (move_effect_journal[curr].type)
    {
      case move_effect_piece_movement:
      ...
      case move_effect_walk_change:
      {
        ...
        context_open(&walk_change_context,move_context->engine,move_context->file,move_context->symbol_table,curr,"[","]");
        write_singlebox_promotion(&walk_change_context,curr);
      ...
      case move_effect_piece_creation:
      case move_effect_piece_readdition:
        if (move_effect_journal[curr].u.piece_addition.added.on<capture_by_invisible)
        {
          ...
          write_piece_creation(&piece_creation_context,curr);
                                 ...
                                 output_symbol_table_type const *symbol_table)
{
  ...
  write_pre_capture_effects(&context);
